...
describe('Celts', () => {
  test('there are 28 Celts cards', () => {
    expect(cards.length).toEqual(28);
  });

  test('there are 16 cards with no icon', () => {
    const cardsWithNoIcon = cards.filter(card => card.icon === undefined);
    expect(cardsWithNoIcon.length).toEqual(16);
  });
  ...
  test('there are 14 cards with no suit', () => {
    const cardsWithNoSuit = cards.filter(card => card.suit === undefined);
    expect(cardsWithNoSuit.length).toEqual(14);
  });

  test('there are 8 cards with Unrest suit', () => {
    const cardsWithUnrestSuit = cards.filter(card => card.suit === Suit.UNREST);
    expect(cardsWithUnrestSuit.length).toEqual(8);
  });

  test('there are 18 cards with no state', () => {
    const cardsWithNoState = cards.filter(card => card.state === undefined);
    expect(cardsWithNoState.length).toEqual(18);
  });
  ...
  test('there are 5 cards with Empire state', () => {
  ...
  test('there are 6 cards in Nation deck', () => {
